...
public class DatabaseDocumentBuilder {
    ...
    private static List<TableInfo> filterTableInfo(List<TableInfo> tables) {
        Map<String, TableInfo> map
                = tables.stream()
                .collect(Collectors.toMap(TableInfo::getTableName, table -> table));

        List<String> exportTableNames = config.getExportTableNames();
        if (exportTableNames == null || exportTableNames.isEmpty()) {
            return tables;
        }
